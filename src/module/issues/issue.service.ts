...
const creatissueIntoBD=async(payload:IIssue)=>{
...
const getSingleIssueIntoBD = async (id: string) => {
    const result = await pool.query(
        `
        SELECT
            issues.id,
...
const updateIssueIntoBD = async (id: number, payload: IIssue) => {
...
const deleteIssueIntoBD = async (id: number) => {
    const result = await pool.query(
        `
        DELETE FROM issues
        WHERE id = $1
